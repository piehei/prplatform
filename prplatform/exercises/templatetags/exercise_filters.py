...
def my_received_feedback_count(exercise, user):
    ...
    if exercise.use_groups:
        return exercise.submissions.filter(
                reviewed_submission__submitter_group=exercise.course.find_studentgroup_by_user(user)).count()
    return exercise.submissions.filter(reviewed_submission__submitter_user=user).count()
