...
class ReviewExercise(BaseExercise):
    ...
    can_review_own_submission = models.BooleanField("Students can peer-review themselves", default=False)
    ...
    def get_choose_type_queryset(self, user):
        ...
        if self.type == ReviewExercise.GROUP:
            group = self.course.find_studentgroup_by_user(user)
            if group:
                qs = qs.filter(submitter_user__email__in=group.student_usernames)
